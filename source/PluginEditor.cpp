...
void ScopeDisplay::paint(Graphics& g)
{
    ...
    g.setColour(displayA().withAlpha(0.13f));
    g.strokePath(front, PathStrokeType(18.0f, PathStrokeType::curved, PathStrokeType::rounded));
    g.setColour(displayB().withAlpha(0.20f));
    g.strokePath(front, PathStrokeType(9.0f, PathStrokeType::curved, PathStrokeType::rounded));
    g.setColour(displayA());
    g.strokePath(front, PathStrokeType(2.6f, PathStrokeType::curved, PathStrokeType::rounded));
    ...
    for (int i = 0; i < 36; ++i)
    {
        ...
        const float sz = 1.5f + 3.8f * std::abs(std::sin(t * MathConstants<float>::twoPi));
...
void SerumKnob::setLinearStyle(bool shouldBeLinear)
{
    ...
    if (linearStyle)
    {
        ...
        slider.setTextBoxStyle(Slider::TextBoxRight, false, 72, 20);
...
void SerumKnob::resized()
{
    ...
    if (linearStyle)
    {
        title.setBounds(r.removeFromLeft(58));
    ...
    : AudioProcessorEditor(&p), audioProcessor(p), scope(p), spectrum(p), lfoShape(p), matrixView(p)
{
    ...
    logo.setText("5PARKS BASSLAB X", dontSendNotification);
    logo.setColour(Label::textColourId, textHi());
    logo.setFont(FontOptions(28.0f, Font::bold));
    subtitle.setText("modern bass instrument · heavyweight low end · motion macros · club-first voicing · flagship toybox", dontSendNotification);
...
void FiveParksVST3AudioProcessorEditor::paint(Graphics& g)
{
    ...
    drawSection(g, displayBox.getBounds().toFloat(), "ENGINE VIEW · WAVE · BODY · MOTION", displayA());
    ...
    drawSection(g, bassBox.getBounds().toFloat(), "MACRO LAB", bassCol());
    drawSection(g, envBox.getBounds().toFloat(), "AMP SHAPE", envCol());
    drawSection(g, lfoBox.getBounds().toFloat(), "MOTION", lfoCol());
    drawSection(g, fxBox.getBounds().toFloat(), "FX", fxCol());
    drawSection(g, perfBox.getBounds().toFloat(), "PLAY", perfCol());
    drawSection(g, matrixBox.getBounds().toFloat(), "MOD MATRIX", modCol());
    ...
    auto drawChip = [&](juce::Rectangle<float> r, const juce::String& text, juce::Colour c)
    {
        ...
        g.drawText(text, r.toNearestInt(), Justification::centred);
    };

    auto chipRow = juce::Rectangle<float>((float) getWidth() - 430.0f, 18.0f, 400.0f, 28.0f);
    drawChip(chipRow.removeFromLeft(126.0f), "LOW END", subCol());
    chipRow.removeFromLeft(10.0f);
    drawChip(chipRow.removeFromLeft(126.0f), "MOTION", lfoCol());
    chipRow.removeFromLeft(10.0f);
    drawChip(chipRow.removeFromLeft(126.0f), "TOYBOX", fxCol());

    auto strip = Rectangle<float>(20.0f, (float) getHeight() - 18.0f, (float) getWidth() - 40.0f, 4.0f);
...
void FiveParksVST3AudioProcessorEditor::resized()
{
    ...
    auto heroBand = area.removeFromTop(398);
    auto leftHero = heroBand.removeFromLeft((int) (heroBand.getWidth() * 0.66f));
    displayBox.setBounds(leftHero);
    heroBand.removeFromLeft(10);
    auto rightHero = heroBand;
    perfBox.setBounds(rightHero.removeFromTop(122));
    rightHero.removeFromTop(10);
    filterBox.setBounds(rightHero.removeFromTop(126));
    rightHero.removeFromTop(10);
    auto stackBox = rightHero;
    subBox.setBounds(stackBox.removeFromTop((stackBox.getHeight() - 10) / 2));
    stackBox.removeFromTop(10);
    noiseBox.setBounds(stackBox);

    area.removeFromTop(10);
    auto row1 = area.removeFromTop(250);
    oscABox.setBounds(row1.removeFromLeft(360));
    row1.removeFromLeft(10);
    oscBBox.setBounds(row1.removeFromLeft(360));
    row1.removeFromLeft(10);
    bassBox.setBounds(row1);

    area.removeFromTop(10);
    auto row2 = area;
    envBox.setBounds(row2.removeFromLeft(270));
    row2.removeFromLeft(10);
    lfoBox.setBounds(row2.removeFromLeft(270));
    row2.removeFromLeft(10);
    fxBox.setBounds(row2.removeFromLeft(360));
    row2.removeFromLeft(10);
    ...
    sb.removeFromTop(12);
    subLevel.setBounds(sb.removeFromTop(40).withWidth(sb.getWidth() - 8));
    ...
    nb.removeFromTop(12);
    noiseLevel.setBounds(nb.removeFromTop(40).withWidth(nb.getWidth() - 8));
